...
esp_err_t usb_camera_init(size_t width, size_t height)
{
    ...
    // Wait for device to be connected and initialized
    vTaskDelay(2000 / portTICK_PERIOD_MS);

    // Check if device is ready
    if (g_uvc_dev == NULL || g_uvc_dev->event_group == NULL) {
        ESP_LOGW(TAG, "Camera device not ready, stream will start when device is connected");
        return ESP_FAIL;
    }
