...
namespace matrix_print {

// Generic matrix printing function - automatically gets dimensions from template parameters
template<int rows, int cols>
inline void print(const Matrixf<rows, cols> &mat, const char* name = "Matrix")
{
    std::cout << "\n" << name << " (" << rows << "x" << cols << "):\n";

    // Calculate border width based on number of columns
    int border_width = cols * 16; // 12 chars per number + 4 spaces
    ...
    for (int i = 0; i < rows; i++) {
        ...
        for (int j = 0; j < cols; j++) {
            std::cout << std::setw(12) << std::fixed << std::setprecision(6) << mat[i][j] << " ";
        }
        std::cout << "│\n";
    }
...
// Convenience functions for common matrix types (using the generic function)
inline void print_matrix4x4(const Matrixf<4, 4> &mat, const char* name = "Matrix")
{
    print(mat, name);
}

inline void print_vector3x1(const Matrixf<3, 1> &vec, const char* name = "Vector")
{
    print(vec, name);
}

inline void print_vector6x1(const Matrixf<6, 1> &vec, const char* name = "Vector")
{
    print(vec, name);
}
...
template<int rows, int cols>
inline void print_matrix(const Matrixf<rows, cols> &mat, const char* name = "Matrix")
{
    print(mat, name);
}
